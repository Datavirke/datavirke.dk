...
of tools we're using at the moment, I've decided to compile all of them into a singular container
...
mellowed out a bit in my old age, and the trauma of dealing with all manner of OpenSSL, musl, and DNS-related
...
larger than `alpine:latest`'s meagre 7.33MB, but 74MB is still practically nothing, and if you're diligent
...
To make copying from one stage to another easier, we'll put all the binaries
...
But of course we won't be staying on that version for long. `1.27.5` and `1.28.1` have just been released,
...
## Workspace Stage
With all our binaries rolled up into a nice little _installer_ stage, we can start configuring a separate stage 
...
Running as root is of course considered _haram_. Perhaps more importantly, our repository will be cloned
...
it *outside* of the container. Security implications aside, using root is just plain tedious!
...
Next, copy the binaries over from our installer stage and **ch**ange the **own**er:
...
With our Dockerfile (current version available [here](https://github.com/MathiasPius/kronform/tree/main/tools)) setup,
...
for making it a little easier for myself. Even if you're not familiar with [Just](https://github.com/casey/just), you should be
able to glean what's going.
...
so as to not repeat the massive blunder that was [The First Incident](/posts/bare-metal-kubernetes-first-incident/).

> `--preserve` flag was removed and is always implicitly enabled since [1.8.0](https://github.com/siderolabs/talos/releases/tag/v1.8.0).
{: .prompt-tip }
