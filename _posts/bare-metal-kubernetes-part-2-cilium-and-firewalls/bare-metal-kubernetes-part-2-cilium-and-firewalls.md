...
date: 2023-06-23 08:00:00 CEST
tags: ["hetzner", "talos", "cilium", "kubernetes", "bare-metal", "cni"]
...
```
<small><i>In Part III I'll translate this into a FluxCD `HelmChart` so we can manage it in git and keep it updated.</i></small>

> **Ghost of Clusters Past 👻**
> 
> This was the first of many times overriding the cluster's DNS domain got in my way. If you're following along at home, but didn't heed my warning, there's still time for you!
>
> Go back to [Part I](/posts/bare-metal-kubernetes-part-1-talos-on-hetzner) and start over!
{: .prompt-danger }
...
* `hubble.peerService.clusterDomain` because I changed the cluster domain.
* `etcd.clusterDomain` same as above.
* `hubble.relay.enabled` because Hubble is great for debugging network policy errors.
* `hubble.relay.enabled` same as above.
* `hostFirewall.enabled` so we can use `CiliumClusterWideNetworkPolicy` for firewalling.
...
![Hetzner Firewall Rules]({{ page.url }}hetzner-rules.png)
...
### Policy Audit Mode
When applying network policies to nodes it's entirely possible that I end up breaking Talos' ability to communicate with the cluster, or break cluster access in some other funky ways. For this reason it's a good idea to put our node into [Audit Mode](https://docs.cilium.io/en/v1.13/security/host-firewall/#enable-policy-audit-mode-for-the-host-endpoint) which means that packets which violate any of our policies will result in a warning, rather than getting dropped outrght.

This means that we can inspect the audit log and see if our policies are working, without the risk of losing access to the node or cluster.

Enabling audit mode is a little involved since Cilium operates on its own internal representation of the cluster via identities and endpoints, so first we need to figure out what our node's endpoint is. 

In Cilium identities are just 16-bit integers assigned by Cilium, with the lower range reserved for *special* identities which cover things like *hosts* (1), *world* (2), and *kube-apiserver* (7), which lets you easily write policies which target one of these.

Identity `1` is the one reserved for hosts, which is what we want to put into audit mode. We can't put an identity into audit mode though, so first we need to find the endpoint matching the *host* identity. We could install the cilium CLI locally and use that to interact with our setup, but it's much easier to just use the cilium pod already deployed inside our cluster:

```bash
[mpd@ish]$ kubectl exec -n kube-system daemonset/cilium \
...
Cilium uses an enforcement mode to decide how traffic is controlled. The default enforcement mode means that endpoints which don't have any policies associated with them default to allow, which means that until we actually apply a policy, Cilium won't care. Of course you can put Cilium into strict enforcement mode, and we might at a later point, but without any policies assigned, we'd literally break everything by doing so.
...
```bash
[mpd@ish]$ kubectl exec -n kube-system daemonset/cilium -- cilium monitor -t policy-verdict --related-to 555
```

![Welcome to the Internet]({{ page.url }}welcome-to-internet.png)

Welcome to the internet! Your ports *will* be scanned!


The only things I really care about at this stage however, is if our policy allows us to interact with Kubernetes and Talos, so let's give them a ring:
...
In [Part III: Encrypted GitOps with FluxCD](/posts/bare-metal-kubernetes-part-3-encrypted-gitops-with-fluxcd/), we'll setup FluxCD and try to get as much of our code (machineconfigs, helm releases, network policies, etc.) shoved into Git so we can be all ***~GitOpsy~***
