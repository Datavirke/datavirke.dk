...
I went ahead and created [github.com/MathiasPius/kronform](https://github.com/MathiasPius/kronform), and embarked to configure a personal access token, as the guide requires. 
...
> In [Part VIII](/posts/bare-metal-kubernetes-part-8-containerizing-our-work-environment), we create a somewhat reproducible work environment using a Dockerfile,
> so we won't have to install these dependencies locally, and can keep them tightly versioned. This is really handy on a rolling release distribution (I use Arch, by the way).
{: .prompt-tip }
...
> **Ghost of Clusters Past 👻**
> 
> It sure is. This will be your last warning, if you insist on following my footsteps right into quicksand, it's on you!
{: .prompt-danger }
...
Let's just deploy the `GitRepository` and the `Kustomization` resources at the same time.
```yaml
...
```
After a short while, flux picks up our change and starts spamming updates, eventually converging on our desired state:
...
Since we don't have an ingress controller yet (that's in [Part IV](/posts/bare-metal-kubernetes-part-4-ingress-dns-certificates)), we can't expose the service that way, so let's just proxy it from our local machine:
...
I couldn't find any documentation on how such a restructuring might happen, and the cli didn't provide any clues. Looking at the already deployed kustomizations generated during the bootstrap phase, the process seems pretty obvious. Here is `manifests/flux-system/gotk-sync.yaml`:
...
_Unfortunately_ the field doesn't work across flux-managed resource types, meaning we can't define our policy as part of a `Kustomization` and tell it to wait on a `HelmRelease`

_**FORTUNATELY**_ we can work around this limitation using a kustomization [healthcheck](https://fluxcd.io/flux/components/kustomize/kustomization/#health-checks) instead.
...
Next, as per the guide we export the secret part of this key and put it directly into a Kubernetes secret in the `flux-system` namespace, using the fingerprint to grab it using `gpg`:
...
And there we go. Now, **after carefully examining all the now partially encrypted files to ensure no sensitive data is included**, we can go ahead and commit it to git.
...
[mpd@ish]$ export DOCKER_PASSWORD=9LqBV...
```
...
In [Part IV: Ingress, DNS and Certificates](/posts/bare-metal-kubernetes-part-4-ingress-dns-certificates), we'll get some of the basic necessities of a cluster up and running, like an ingress controller, automatic dns record creation with [external-dns](https://github.com/kubernetes-sigs/external-dns) and of course certificate retrieval using [cert-manager](https://cert-manager.io/), so we can expose entire TLS-encrypted websites without ever leaving our cluster!
