...
<small>sha256 hashes omitted for brevity</small>
...
> The stewards of Bitnami were recently acquired by Broadcom, who have decided to go on a _"Make the Entire IT Community Hate us"_ speed run, by [charing $50k for access to Bitnami](https://community.broadcom.com/tanzu/blogs/beltran-rueda-borrego/2025/08/18/how-to-prepare-for-the-bitnami-changes-coming-soon) and [exploding VMWare licensing costs](https://blogs.vmware.com/cloud-foundation/2025/08/29/vmware-cloud-foundation-cloud-on-your-terms/).
{: .prompt-danger }
...
When deploying [Kubernetes' ingress nginx](/posts/bare-metal-kubernetes-part-4-ingress-dns-certificates#deploying-ingress-nginx-with-flux), it also sets up an admission hook that checks the validity of the incoming ingresses, "admitting" ones that are correctly defined. But there's nothing wrong with our Harbor ingress, the problem lies elsewhere.
...
Using our list of container images above as a reference, we setup *Registries* for each, setting up users and providing the authentication details where applicable:

![Mirror Registries]({{ page.url }}registries.png)
...
![Mirror Projects]({{ page.url }}projects.png)
...
![Robot Account]({{ page.url }}robot.png)
...
![Alpine Repository]({{ page.url }}alpine.png)
...
In [Part VIII: Containerizing our Work Environment](/posts/bare-metal-kubernetes-part-8-containerizing-our-work-environment/),
