...
Watching Rook slowly mold itself into a Ceph cluster through `kubectl get all -n rook-ceph` is a real pleasure, when it works!
...
My first thought was that perhaps some empty or corrupt partition table might be messing with the procedure. After all, it worked on one of the nodes so our settings are probably fine. I used the below script to get access on each of the failing nodes, modifying the node-selector as I went along:
...
rook-ceph-osd-0-5dddbd6b66-x294g                1/1     Running     0          9h
rook-ceph-osd-1-dc8bb7f6b-dmljl                 1/1     Running     0          9h
rook-ceph-osd-2-8449684665-p22jp                1/1     Running     0          9h
rook-ceph-osd-3-c695bc8dd-2lhmh                 1/1     Running     0          9h
rook-ceph-osd-4-6f99d7bc47-qxvgv                1/1     Running     0          9h
rook-ceph-osd-5-755689595-m98rt                 1/1     Running     0          9h
rook-ceph-osd-prepare-n1-dp4wg                  0/1     Completed   0          9h
rook-ceph-osd-prepare-n2-shb4d                  0/1     Completed   0          9h
rook-ceph-osd-prepare-n3-bl2vv                  0/1     Completed   0          9h
```
...
With the pool created, we just need to expose it to Kubermetes through a [StorageClass](https://kubernetes.io/docs/concepts/storage/storage-classes/) based on Rook's [own example](https://rook.io/docs/rook/v1.11/Storage-Configuration/Block-Storage-RBD/block-storage/#provision-storage)
```yaml
---
